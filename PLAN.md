...
## Phase 1: Port to NeoForge 26.3 (rewrite, guided by the old code)  — DONE 2026-09-22
Status: commit `777b9f0` on `neoforge-26.3`. Compiles, jar builds, dedicated server loads with no data errors, client loads with no missing models or textures. RCON checks passed: messy bed head is found by `locate poi minecraft:home`, mining a vanilla bed drops the messy bed, `turned_down` follows `time set night` / `day` on both halves within one check interval. Deviations from the design below: no `PlayerWakeUpEvent` handler (the sleeping position is already cleared when it fires; the message is sent from `onStopSleeping` instead), no datagen (resources are generated by `tools/gen_resources.py` and committed), NeoForge loads `data/<ns>/loot_modifiers/*.json` directly so there is no `global_loot_modifiers.json` index any more, and vanilla chest loot tables contain no beds so the loot modifier only matters for modded loot and `/loot`.
